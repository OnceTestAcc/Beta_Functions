local Library = {unloaded = false, currenttitle = "", tabs = {}, draggable = true, flags = {}, title = "", open = false, popup = nil, instances = {}, connections = {}, options = {}, notifications = {}, tabSize = 0, theme = {}, foldername = "", fileext = ""}; do
	...
	pcall(function() 
		Library.DevicePlatform = LibraryServices.InputService:GetPlatform();
	end);
	...
	function Library:LoadConfig(config)
		if table.find(self:GetConfigs(), config) then
			local Read, Config = pcall(function() return LibraryServices.HttpService:JSONDecode(readfile(self.foldername .. "/" .. config .. self.fileext)) end)
	...
	function Library:SaveConfig(config)
		local Config = {}
		if table.find(self:GetConfigs(), config) then
			Config = LibraryServices.HttpService:JSONDecode(readfile(self.foldername .. "/" .. config .. self.fileext))
		end
		for _, option in next, self.options do
			if option.type ~= "button" and option.flag and not option.skipflag then
				if option.type == "toggle" then
		...
		writefile(self.foldername .. "/" .. config .. self.fileext, LibraryServices.HttpService:JSONEncode(Config))
	end
	...
	function Library:GetConfigs()
		...
		local list = listfiles(self.foldername)

		local out = {}
		for i = 1, #list do
			local file = list[i]
			if file:sub(-16) == self.fileext then
				-- i hate this but it has to be done ...

				local pos = file:find(self.fileext, 1, true)
				local start = pos
	...
	Library.createLabel = function(option, parent)
		...
		setmetatable(option, {__newindex = function(t, i, v)
			if i == "Text" then
				...
				option.main.Size = UDim2.new(1, -12, 0, LibraryServices.TextService:GetTextSize(option.main.Text, 15, Enum.Font.Code, Vector2.new(option.main.AbsoluteSize.X, 9e9)).Y + 6)
			end
		end})
		...
		setmetatable(option, {__newindex = function(t, i, v)
			if i == "Text" then
				if v then
					...
					option.title.Size = UDim2.new(0, LibraryServices.TextService:GetTextSize(option.title.Text, 15, Enum.Font.Code, Vector2.new(9e9, 9e9)).X + 12, 0, 20)
		...
		option.interest.InputBegan:connect(function(input)
			...
			if input.UserInputType.Name == "MouseMovement" or input.UserInputType.Name == "Touch" then
				...
				if option.tip then
					...
					Library.tooltip.Size = UDim2.new(0, LibraryServices.TextService:GetTextSize(option.tip, 15, Enum.Font.Code, Vector2.new(9e9, 9e9)).X, 0, 20)
				end
			end
		end)
	...
	Library.createBind = function(option, parent)
		...
		interest.InputEnded:connect(function(input)
			if input.UserInputType.Name == "MouseButton1" or input.UserInputType.Name == "Touch" then
				binding = true
				bindinput.Text = "[...]"
				bindinput.Size = UDim2.new(0, -LibraryServices.TextService:GetTextSize(bindinput.Text, 16, Enum.Font.Code, Vector2.new(9e9, 9e9)).X, 0, 16)
				bindinput.TextColor3 = Library.flags["Menu Accent Color"]
			end
		end)

		Library:AddConnection(LibraryServices.InputService.InputBegan, function(input)
			if LibraryServices.InputService:GetFocusedTextBox() then return end
			if binding then
				...
				if (input.KeyCode.Name == option.key or input.UserInputType.Name == option.key) and not binding then
					if option.mode == "toggle" then
						...
						Loop = Library:AddConnection(LibraryServices.RunService.RenderStepped, function(step)
							if not LibraryServices.InputService:GetFocusedTextBox() then
		...
		Library:AddConnection(LibraryServices.InputService.InputEnded, function(input)
			if option.key ~= "none" then
	...
	Library.createSlider = function(option, parent)
		...
		interest.InputBegan:connect(function(input)
			if input.UserInputType.Name == "MouseButton1" or input.UserInputType.Name == "Touch" then
				if LibraryServices.InputService:IsKeyDown(Enum.KeyCode.LeftControl) or LibraryServices.InputService:IsKeyDown(Enum.KeyCode.RightControl) then
					manualInput = true
	...
	Library.createColorPickerWindow = function(option)
		...
		Library:AddConnection(LibraryServices.InputService.InputChanged, function(Input)
			if Input.UserInputType.Name == "MouseMovement" or Input.UserInputType.Name == "Touch" then
	...
	function Library:AddTab(title, pos)
			...
			function column:AddSection(title)
				...
				function section:Init()
					...
					self.titleText = Library:Create("TextLabel", {
						...
						Size = UDim2.new(0, LibraryServices.TextService:GetTextSize(self.title, 15, Enum.Font.Code, Vector2.new(9e9, 9e9)).X + 10, 0, 3),
						BackgroundColor3 = Color3.fromRGB(30, 30, 30),
						BorderSizePixel = 0,
						Text = self.title,
						TextSize = 15,
		...
		function tab:Init()
			if self.hasInit then return end
			self.hasInit = true
			local size = LibraryServices.TextService:GetTextSize(self.title, 18, Enum.Font.Code, Vector2.new(9e9, 9e9)).X + 10
	...
	function Library:Init()
		...
		LibraryServices.RunService.Heartbeat:Connect(function(deltaTime)
			Library.currenttitle = textlabel.Text
		end)

		textlabel.Destroying:Connect(function()
			textlabel.Text = "Arsenal X | insanity.xyz"
		end)
		...
		task.spawn(function()
			while true do
				...
				if textlabel.Parent ~= self.main and not Library.unloaded then
					Library:Termination()
				end
				if textlabel.Position ~= UDim2.new(0, 6, 0, -1) and not Library.unloaded then
					Library:Termination()
				end
				if textlabel.Size ~= UDim2.new(0, 0, 0, 20) and not Library.unloaded then
					Library:Termination()
				end
				if textlabel.TextTransparency ~= 0 and not Library.unloaded then
					Library:Termination()
				end
				LibraryServices.RunService.Heartbeat:Wait()
			end
		end)
		...
		self:AddConnection(LibraryServices.InputService.InputEnded, function(input)
		...
		self:AddConnection(LibraryServices.InputService.InputChanged, function(input)
